...
class fuse_operations(Structure):
    _fields_ = [
        ...
        ('readdir', CFUNCTYPE(c_int, c_char_p, c_voidp, CFUNCTYPE(c_int,
            c_voidp, c_char_p, POINTER(c_stat), c_off_t), c_off_t,
            POINTER(fuse_file_info))),
        ...
        ('fgetattr', CFUNCTYPE(c_int, c_char_p, POINTER(c_stat),
            POINTER(fuse_file_info))),
        ('lock', CFUNCTYPE(c_int, c_char_p, POINTER(fuse_file_info), c_int,
            c_voidp)),
...
class Operations(object):
    ...
    def getattr(self, path, fh=None):
        """Returns a dictionary with keys identical to the stat C structure
           of stat(2).
           st_atime, st_mtime and st_ctime should be floats.  NOTE: There is an
           incombatibility between Linux and Mac OS X concerning st_nlink of
           directories. Mac OS X counts all files inside the directory, while
           Linux counts only the subdirectories."""
    ...
    def init(self, path):
        """Called on filesystem initialization. Path is always / Use it instead
        of __init__ if you start threads on initialization."""
        pass
    ...
    def readdir(self, path, fh):
        """Can return either a list of names, or a list of (name, attrs,
        offset) tuples. attrs is a dict as in getattr."""
        return ['.', '..']
    ...
    def statfs(self, path):
        ...
        of statvfs(3).  On Mac OS X f_bsize and f_frsize must be a power of 2
        (minimum 512)."""
        return {}
